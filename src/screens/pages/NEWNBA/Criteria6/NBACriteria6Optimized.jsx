...
import CriterionForm from "../Components/CriterionForm6";
...
const NBACriteria6Optimized = () => {
  ...
  const s3DocumentUrl =
    "https://wccdm.s3.ap-south-1.amazonaws.com/nba/1765293737-criterion6_merged.png";
  ...
  const toggleSection = async (id) => {
    ...
    // If expanding a section and it's 6.1 or 6.2, fetch card details for live updates
    if (!wasExpanded) {
      ...
      if (indicator && indicator.rawName && (indicator.rawName.includes("6.1") || indicator.rawName.includes("6.2"))) {
        ...
        console.log("❌ DEBUG: Not a 6.1 or 6.2 section or no indicator found");
      }
    }
  };
  ...
  const handleCardClick = async (subLevel2Id, userStaffId, cardItem = null) => {
    try {
      ...
      const selectedCardData = {
        ...
        editMode: true,
        professionalDevelopmentId: cardItem?.[cardConfig.idField] || null,
        developmentActivitiesId: cardItem?.[cardConfig.idField] || null, // For 6.2 sections
        cardData: cardItem,
        sectionName: sectionName
      };
      setSelectedCard(selectedCardData);
    } catch (error) {
      console.error("❌ CRITERIA 6 - Error in handleCardClick:", error);
  ...
  const cleanDisplayName = (rawName) => {
    ...
    return rawName.trim().replace(/^6\.\d+\s*-?\s*/i, "").trim() || "Untitled Section";
  };
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const response = await ReportService.getReportCri6(cycleSubCategoryIds);
      ...
      link.href = url;
      link.download = `Criterion6_Report_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  if (loading) {
    return <CriteriaLoader criterionNumber={6} progress={progress} />;
  }
          ...
          <div className="space-y-6">
            {keyIndicators.length === 0 ? (
              ...
              keyIndicators.map((indicator, index) => {
                ...
                let sectionCode = `6.${sectionNumber}`; // fallback
                if (indicator.rawName) {
                  const match = indicator.rawName.match(/6\.(\d+)/);
                  if (match) {
                    sectionCode = `6.${match[1]}`;
                  }
                }
                console.log("🔍 DEBUG: Section mapping - index:", index, "rawName:", indicator.rawName, "sectionCode:", sectionCode);
                              ...
                              {(() => {
                                ...
                                return shouldShow;
                                  ...
                                  <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-6">
                                    {console.log("🎯 CRITERIA 6 - FORM IS RENDERING! This div should be visible.")}
                                    ...
                                    <CriterionForm
                                      ...
                                      professionalDevelopmentId={selectedCard.professionalDevelopmentId || null}
                                      developmentActivitiesId={selectedCard.developmentActivitiesId || null}
                                      isSubCoordinator={isSubCoordinator}
                                    />
                                  </div>
                                )}
...
export default NBACriteria6Optimized;