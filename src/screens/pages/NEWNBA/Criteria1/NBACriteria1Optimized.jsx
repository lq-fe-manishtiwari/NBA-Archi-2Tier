...
import CriterionForm from "../Components/Criteria1/CriterionForm";
...
const NBACriteria1Optimized = () => {
  ...
  useEffect(() => {
    ...
    console.log("🟣 NBACriteria1Optimized - Received route state:", stateFromRoute);
    console.log("  - programId from state:", stateFromRoute.programId);
    console.log("  - nba_accredited_program_id:", stateFromRoute.nba_accredited_program_id);
    console.log("  - programId extracted:", programId);
  ...
  const fetchContributorAllocations = async () => {
    try {
      // Fast-path when coming from contributor dashboard with task data
  ...
  const fetchCardDetails = async (cycleSubCategoryId) => {
    ...
    try {
      ...
      // Fetch contributor cards using appropriate API
      console.log("📡 DEBUG: Fetching contributor cards for section:", sectionName);
      let cardDetails = [];
      
      if (sectionName.includes("1.1")) {
        cardDetails = await newnbaCriteria1Service.getallCardDetails(cycleSubCategoryId);
      } else if (sectionName.includes("1.2")) {
        cardDetails = await newnbaCriteria1Service.getAllCriteria1_2_Data?.(cycleSubCategoryId) || [];
      } else if (sectionName.includes("1.3")) {
        cardDetails = await newnbaCriteria1Service.getAllCriteria1_3_Data?.(cycleSubCategoryId) || [];
      } else if (sectionName.includes("1.4")) {
        cardDetails = await newnbaCriteria1Service.getAllCriteria1_4_Data?.(cycleSubCategoryId) || [];
      } else if (sectionName.includes("1.5")) {
        cardDetails = await newnbaCriteria1Service.getAllCriteria1_5_Data?.(cycleSubCategoryId) || [];
      } else {
        cardDetails = await newnbaCriteria1Service.getallCardDetails(cycleSubCategoryId);
      }
      ...
      console.log("👤 DEBUG: currentUserInfo2:", currentUserInfo2);
      ...
      if (currentStaffId) {
        try {
          ...
          const coordinatorData = await newnbaCriteria1Service.getCriteria1_1_Data(cycleSubCategoryId, currentStaffId);
          ...
          if (coordinatorRecord && coordinatorRecord.vision_mission_peo_id) {
            ...
            if (!existingCard) {
              ...
              const coordinatorCard = {
                ...
                approval_status: coordinatorRecord.approval_status || 'COORDINATORS_DATA', // Use existing status or default
  ...
  const handleCardClick = async (subLevel2Id, userStaffId, cardItem = null) => {
    try {
      ...
      console.log("  - cardItem.other_staff_id:", cardItem?.other_staff_id);
      console.log("  - cardItem.is_coordinator_entry:", cardItem?.is_coordinator_entry);
      ...
      console.log("🔴 DEBUG selectedCard set with otherStaffId:", userStaffId);
    } catch (error) {
  ...
  const fetchAllIndicatorData = async (indicators) => {
    ...
    // Fast-path for contributor with task data
    if (stateFromRoute.fromContributorDashboard && stateFromRoute.taskData) {
      ...
      setCompletedStatus({ [subLevel2Id]: taskData.status === "COMPLETED" });
      setDataFilledStatus({ [subLevel2Id]: true });
  ...
  const toggleSection = async (id) => {
    ...
    // If expanding a section and it's 1.1, fetch card details for live updates
    if (!wasExpanded) {
      const indicator = keyIndicators.find(ind => ind.subLevel2Id === id);
      console.log("🔍 DEBUG: Found indicator:", indicator);

      if (indicator && indicator.rawName && (indicator.rawName.includes("1.1") || indicator.rawName.includes("1.2") || indicator.rawName.includes("1.3") || indicator.rawName.includes("1.4") || indicator.rawName.includes("1.5"))) {
        console.log("✅ DEBUG: This is a 1.1/1.2/1.3/1.4/1.5 section, fetching card details...");
        await fetchCardDetails(id);
      } else {
        console.log("❌ DEBUG: Not a 1.1/1.2/1.3/1.4/1.5 section or no indicator found");
      }
    }
  };
  ...
  const cleanDisplayName = (rawName) => {
    ...
    return rawName.trim().replace(/^\d+\.\d+\s*-?\s*/i, "").trim() || rawName.trim() || "Untitled Section";
  };
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const response = await ReportService.getReportCri1(cycleSubCategoryIds);
      ...
      link.href = url;
      link.download = `Criterion1_Report_${new Date().toISOString().split('T')[0]}.pdf`;
...
const fetchMoMarks = async (subLevel2Id) => {
  ...
  if (loading) {
    return <CriteriaLoader criterionNumber={1} progress={progress} />;
  }
          ...
          <div className="space-y-6">
            {keyIndicators.length === 0 ? (
              ...
              keyIndicators.map((indicator, index) => {
                ...
                const actualSectionCode = sectionMatch ? sectionMatch[1] : `1.${index + 1}`;
                ...
                // Contributors only see their allocated sections
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="px-6 py-8 bg-gray-50 border-t border-gray-200">
                        {/* 1. Contributor → always show form */}
                        {isContributor && (
                          (() => {
                        ...
                        {!isContributor && (
                          <>
                            {hasAllocation ? (
                                  ...
                                  <GenericCardWorkflow
                                    ...
                                    apiService={{
                                      updateCardStatus: (payload) => {
                                        ...
                                        return newnbaCriteria1Service.updateCardStatus(payload);
                                      },
                                  ...
                                  {(() => {
                                    ...
                                    <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-6">
                                      ...
                                      <CriterionForm
                                        ...
                                        visionMissionPeoId={selectedCard.visionMissionPeoId || null}
                                      />
                                    </div>
                                  )}
                                </div>
                              ...
                              /* No allocation → coordinator fills himself */
                              (() => {
                                const userInfo = JSON.parse(localStorage.getItem("userInfo") || "{}");
...
export default NBACriteria1Optimized;