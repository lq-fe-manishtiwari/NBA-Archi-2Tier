// src/screens/pages/NEWNBA/Criteria2/NBACriteria2Optimized.jsx
...
import CriterionForm2 from "../Components/Criteria2/CriterionForm2";
...
const NBACriteria2Optimized = () => {
  ...
  const s3DocumentUrl =
    "https://wccdm.s3.ap-south-1.amazonaws.com/nba/1765292019-merged_vertical_2_series.png";
  ...
  const fetchContributorAllocations = async () => {
    try {
      ...
      if (nbaCriteriaSubLevel1Id && stateFromRoute.fromContributorDashboard) {
        filtered = filtered.filter(item => String(item.id) === String(nbaCriteriaSubLevel1Id));
      }
  ...
  const fetchAllIndicatorData = async (indicators) => {
    ...
    const currentStaffId = userInfo.school_user_id || userInfo.user_id;
    ...
    for (const ind of indicators) {
      ...
      try {
        if (isContrib) {
          if (!currentStaffId && stateFromRoute.taskData && stateFromRoute.taskData.contributor) {
            const c = stateFromRoute.taskData.contributor;
            users = [{
              school_user: {
              ...
              nba_contributor_allocation_id: stateFromRoute.taskData.allocation_id,
              completed: false,
            }];
          } else {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const response = await ReportService.getReportCri2(cycleSubCategoryIds);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Criterion2_Report_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  if (loading) {
    return <CriteriaLoader criterionNumber={2} progress={progress} />;
  }
          ...
          <div className="space-y-6">
            {keyIndicators.length === 0 ? (
              ...
              keyIndicators.map((indicator, index) => {
                ...
                const actualSectionCode = sectionMatch ? sectionMatch[1] : `2.${index + 1}`;
                ...
                if (isContributor && !isSubCoordinator && !hasAllocation && !stateFromRoute.fromContributorDashboard) return null;
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            {hasAllocation && (!isContributor || isSubCoordinator) && (
                      ...
                      <div className="px-6 py-8 bg-gray-50 border-t border-gray-200">
                        {isContributor && (
                          (() => {
                            ...
                            const otherStaffId = users[0]?.school_user?.school_user_id ||
