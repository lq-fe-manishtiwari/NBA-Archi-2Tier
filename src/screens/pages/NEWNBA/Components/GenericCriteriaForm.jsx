// src/screens/pages/NEWNBA/GenericCriteriaForm.jsx
...
// Generic Table
const GenericTable = ({ columns, data = [], onChange, disabled, fieldId = "default", readOnlyRows = [] }) => {
                  ...
                  <Draggable key={row.id} draggableId={row.id.toString()} index={i} isDragDisabled={disabled}>
                    {(provided, snapshot) => (
                      ...
                      >
                        ...
                        {columns.map((col) => {
                          const isRowReadOnly = readOnlyRows.includes(i);
                          const isFieldDisabled = disabled || isRowReadOnly;
                          const fieldClassName = `w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${isRowReadOnly ? 'bg-gray-100 cursor-not-allowed' : ''}`;
                          ...
                          return (
                            <td key={col.field} className="p-3">
                              ...
                              ) : col.type === "date" || col.field.toLowerCase().includes('date') ? (
                                <input
                                  type="date"
                                  value={row[col.field] || ""}
                                  onChange={(e) => handleChange(i, col.field, e.target.value)}
