...
const Criterion1_3Form = ({
  ...
  const loadData = async () => {
    ...
    console.log("🟠 Criterion1_3Form - useEffect triggered:");
    console.log("  - cycle_sub_category_id:", cycle_sub_category_id);
    console.log("  - currentOtherStaffId:", currentOtherStaffId);
    console.log("  - isEditable:", isEditable);

    if (!cycle_sub_category_id) {
      console.log("❌ Criterion1_3Form: cycle_sub_category_id is missing, exiting");
      setLoading(false);
      return;
    }
    ...
    setInitialData({
      ...
      filesByField: {
        "1.3.1": (d.course_documents || []).length > 0 
          ...
          : [{ id: `file-1.3.1-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }],
        "1.3.2": (d.mapping_documents || []).length > 0 
          ...
          : [{ id: `file-1.3.2-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }],
      }
    });

    console.log("✅ Criterion1_3Form: Data loaded and set successfully");
    setLoading(false);
  };
  ...
  const handleSave = async (formData) => {
    ...
    console.log("🟠 Criterion1_3Form handleSave called");
    setSaving(true);
    ...
    try {
      ...
      const filesWithCategory = Object.keys(formData.filesByField || {}).flatMap(fieldName => {
        return (formData.filesByField[fieldName] || []).map(file => {
          let category = "Other";
          if (fieldName === "1.3.1") category = "Course Information";
          if (fieldName === "1.3.2") category = "PO-PSO Mapping";
  ...
  console.log("🎯 Criterion1_3Form rendering with initialData:", initialData);
