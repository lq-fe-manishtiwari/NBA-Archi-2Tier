// src/screens/pages/NEWNBA/Components/Criteria1/GenericCriteriaForm1_4.jsx
...
import { COService } from "../../../OBE/Settings/Services/co.service";
...
const GenericCriteriaForm1_4 = (props) => {
          ...
          <div key={field.name} className="space-y-6">
            {field.type === "heading" ? (
              ...
              <>
                ...
                {/* Custom Content */}
                {field.type === "custom" && field.name === "1.4.1" ? (
                  <div className="space-y-6">
                    {(props.courseOutcomes || []).length > 0 ? (
                      <div className="space-y-6">
                        <h4 className="text-lg font-semibold text-blue-600">Course Outcomes by Semester</h4>
                        {(() => {
                          const semesterGroups = (props.courseOutcomes || []).reduce((acc, co) => {
                            const semesterName = co.semester?.name || 'Unknown Semester';
                            if (!acc[semesterName]) acc[semesterName] = {};
                            const subjectKey = `${co.subject?.name || 'Unknown'} (${co.subject?.subject_code || 'N/A'})`;
                            if (!acc[semesterName][subjectKey]) acc[semesterName][subjectKey] = [];
                            acc[semesterName][subjectKey].push(co);
                            return acc;
                          }, {});
                          
                          return Object.entries(semesterGroups).map(([semesterName, subjects]) => (
                            <div key={semesterName} className="border rounded-lg overflow-hidden mb-4">
                              <table className="w-full border">
                                <thead>
                                  <tr className="bg-[#2163c1] text-white">
                                    <th className="border px-3 py-2 text-left" colSpan="2">{semesterName}</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {Object.entries(subjects).map(([subjectName, courses]) => [
                                    <tr key={`${semesterName}-${subjectName}`} className="bg-blue-100">
                                      <td className="border px-3 py-2 font-semibold" colSpan="2">{subjectName}</td>
                                    </tr>,
                                    <tr key={`${semesterName}-${subjectName}-header`} className="bg-gray-100">
                                      <th className="border px-3 py-2 text-left">CO Code</th>
                                      <th className="border px-3 py-2 text-left">CO Statement</th>
                                    </tr>,
                                    ...courses.map((co, idx) => (
                                      <tr key={co.co_id} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                        <td className="border px-3 py-2 text-center font-medium">{co.co_code}</td>
                                        <td className="border px-3 py-2 text-sm">{co.co_statement || '-'}</td>
                                      </tr>
                                    ))
                                  ]).flat()}
                                </tbody>
                ...
                ) : field.type === "custom" && field.name === "1.4.2" ? (
                  <div className="space-y-6">
                    {Object.keys(props.courseDataMap || {}).length > 0 && (props.poData || []).length > 0 ? (
                      <div className="space-y-6">
                        <h4 className="text-lg font-semibold text-green-600">CO-PO Mapping Matrix</h4>
                        {Object.entries(props.courseDataMap || {}).map(([courseId, data]) => (
                          <div key={courseId} className="border rounded-lg overflow-hidden mb-4">
                            <div className="bg-[#2163c1] text-white px-4 py-2">
                              <h5 className="font-semibold">{data.courseName} ({data.courseCode})</h5>
                            </div>
                            <div className="overflow-x-auto">
                              <table className="min-w-full border text-sm">
                                <thead className="bg-blue-100">
                                  <tr>
                                    <th className="border px-3 py-2">CO Code</th>
                                    <th className="border px-3 py-2">CO Description</th>
                                    {(props.poData || []).map((po) => (
                                      <th key={po.po_id} className="border px-3 py-2">{po.po_code}</th>
                                    ))}
                                    {(props.psoData || []).map((pso) => (
                                      <th key={pso.pso_id} className="border px-3 py-2">{pso.pso_code}</th>
                                    ))}
                                  </tr>
                                </thead>
                                <tbody>
                                  {(data.coData || []).map((co, coIdx) => {
                                    const coMappings = (data.coPoMappingData || []).filter(mapping => 
                                      String(mapping.co?.coId) === String(co.coId)
                                    );
                                    
                                    return (
                                      <tr key={co.coId} className={coIdx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                        <td className="border px-3 py-2 text-center font-medium">{co.coCode}</td>
                                        <td className="border px-3 py-2 text-sm">{co.coStatement || '-'}</td>
                                        
                                        {(props.poData || []).map((po) => {
                                          const mapping = coMappings.find(m => m.po && String(m.po.poId) === String(po.po_id));
                                          return (
                                        ...
                                        {(props.psoData || []).map((pso) => {
                                          const mapping = coMappings.find(m => m.pso && String(m.pso.psoId) === String(pso.pso_id));
                                          return (
                ...
                {field.hasTable ? (
                  <GenericTable
                    ...
                    onChange={(newData) => {
                      setTableData((prev) => ({ ...prev, [field.name]: newData }));
                    }}
                    disabled={!isEditMode}
                    fieldId={field.name}
                  />
                ...
                {!props.isCompleted && field.hasFile && (
...
export default GenericCriteriaForm1_4;