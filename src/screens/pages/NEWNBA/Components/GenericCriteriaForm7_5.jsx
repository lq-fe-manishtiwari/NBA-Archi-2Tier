...
const GenericTable = ({ columns, data = [], onChange, disabled, tableConfig, predefinedRows }) => {
  ...
  const handleChange = (i, field, val) => {
    ...
    const a1 = parseFloat(updated[3].caym1) || 0;
    const a2 = parseFloat(updated[3].caym2) || 0;
    const a3 = parseFloat(updated[3].caym3) || 0;

    const avg = (a1 + a2 + a3) / 3;
    const averageAPI = isFinite(avg) ? avg.toFixed(2) : "0.00";
          ...
          {!isCompleted && (
            <button
              ...
              className={`p-4 rounded-xl transition-all shadow-lg flex items-center justify-center ${
                isEditMode 
                  ? "bg-white hover:bg-gray-200 text-[#2163c1]" 
                  : "bg-white hover:bg-gray-100 text-[#2163c1]"
              }`}
          ...
          <div key={field.name} className="space-y-6">
            ...
            {field.hasTable ? (
              <GenericTable
                ...
                predefinedRows={field.predefinedRows}
              />
            ) : (
...
export default GenericCriteriaForm7_5;