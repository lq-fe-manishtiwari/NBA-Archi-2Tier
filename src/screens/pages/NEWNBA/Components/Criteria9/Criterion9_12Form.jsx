...
const Criterion9_12Form = ({
  ...
  otherStaffId = null,
  editMode = false,
  innovative_initiatives_id: propEducationalInitiativesId = null,
}) => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [educationalInitiativesId, setEducationalInitiativesId] = useState(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🎯 Criterion9_12Form - Loading data with:");
      ...
      console.log("  - propEducationalInitiativesId:", propEducationalInitiativesId);
      ...
      console.log("🎯 Criterion9_12Form - Final staffId:", staffIdToUse);

      if (!staffIdToUse) {
        console.log("❌ Criterion9_12Form - No staffId found, using empty data");
        setInitialData({ 
          content: { initiatives_description: "" }, 
          tableData: [], 
          filesByField: {} 
        });
        setLoading(false);
      ...
      console.log("📡 Criterion9_12Form - Making API call with:");
      ...
      console.log("📊 Criterion9_12Form - Raw API Response:", response);
      ...
      if (Array.isArray(response)) {
        if (response.length > 0) {
          dataItem = response[0];
          console.log("📦 Criterion9_12Form - Extracted first item from array:", dataItem);
        } else {
          console.log("📭 Criterion9_12Form - Empty array response, no data found");
        }
      } else {
        dataItem = response;
      }
      ...
      if (dataItem && dataItem.innovative_initiatives_id) {
        console.log("✅ Criterion9_12Form - Found existing data");
        ...
        // Set ID
        setEducationalInitiativesId(dataItem.innovative_initiatives_id);
        ...
        const filesArray = Array.isArray(dataItem.initiatives_documents) 
          ? dataItem.initiatives_documents 
          : [];

        const filesByField = {
          "initiatives_description": filesArray.length > 0
            ? filesArray.map((f, i) => ({
                id: f.id || `file-educational-${i}`,
        ...
        setInitialData({
          content: { 
            initiatives_description: dataItem.initiatives_description || "" 
          },
          tableData: [],
        ...
        console.log("📭 Criterion9_12Form - No existing data found, showing blank form");
        setEducationalInitiativesId(null);
  ...
  }, [nba_accredited_program_id, nba_criteria_sub_level2_id, nba_contributor_allocation_id, otherStaffId, propEducationalInitiativesId, editMode]);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        initiatives_description: formData.content?.initiatives_description || "",
        initiatives_documents: filesWithCategory
      ...
      console.log("🟠 Criterion9_12Form - Save payload:", payload);

      let response;
      const hasExistingEntry = educationalInitiativesId || propEducationalInitiativesId;

      if (hasExistingEntry) {
        // Update existing record
        const idToUse = educationalInitiativesId || propEducationalInitiativesId;
        console.log("🔄 Criterion9_12Form - Updating existing entry with ID:", idToUse);
        response = await newnbaCriteria9Service.updateCriteria9('9.12', idToUse, payload);
      } else {
        // Create new record
        console.log("🆕 Criterion9_12Form - Creating new entry");
        response = await newnbaCriteria9Service.saveCriteria9Data('9.12', payload);
      }
      ...
      Object.keys(formData.filesByField || {}).forEach(field => {
        ...
        updatedFilesByField[field] = files.map(file => {
          ...
          const savedFile = payload.initiatives_documents?.find(
      ...
      if (response?.innovative_initiatives_id && !educationalInitiativesId) {
        setEducationalInitiativesId(response.innovative_initiatives_id);
      }
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Criterion 9.12 saved successfully
        </SweetAlert>
      );
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria9Service.deleteCriteria9('9.12', educationalInitiativesId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Educational Initiatives record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setEducationalInitiativesId(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <GenericCriteriaForm
        ...
        onSave={handleSave}
        onDelete={educationalInitiativesId ? handleDelete : null}
        isCompleted={completed}
        isContributorEditable={isContributorEditable}
        saving={saving}
        hasExistingData={!!educationalInitiativesId}
...
export default Criterion9_12Form;