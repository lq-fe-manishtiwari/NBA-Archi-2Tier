...
const Criterion9_5Form = ({
  ...
  otherStaffId = null,
  editMode = false,
  startup_entrepreneurship_id: propStartupEntrepreneurshipId = null,
}) => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [startupEntrepreneurshipId, setStartupEntrepreneurshipId] = useState(null);
    ...
    fields: [
      {
        ...
        label: "Describe the initiatives, facilities creation and their effectiveness in encouraging students for innovation, entrepreneurship, incubation and start-up. Also provide the list of beneficiaries.",
        marks: 5,
        type: "textarea"
      }
    ]
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🎯 Criterion9_5Form - Loading data with:");
      ...
      console.log("  - propStartupEntrepreneurshipId:", propStartupEntrepreneurshipId);
      ...
      console.log("🎯 Criterion9_5Form - Final staffId:", staffIdToUse);

      if (!staffIdToUse) {
        console.log("❌ Criterion9_5Form - No staffId found, using empty data");
        setInitialData({ 
          content: { startup_description: "" }, 
          tableData: [], 
          filesByField: {} 
        });
        setLoading(false);
      ...
      console.log("📡 Criterion9_5Form - Making API call with:");
      ...
      console.log("📊 Criterion9_5Form - Raw API Response:", response);
      ...
      if (Array.isArray(response)) {
        if (response.length > 0) {
          dataItem = response[0];
          console.log("📦 Criterion9_5Form - Extracted first item from array:", dataItem);
        } else {
          console.log("📭 Criterion9_5Form - Empty array response, no data found");
        }
      } else {
        dataItem = response;
      }
      ...
      if (dataItem && dataItem.startup_entrepreneurship_id) {
        console.log("✅ Criterion9_5Form - Found existing data");
        ...
        // Set ID
        setStartupEntrepreneurshipId(dataItem.startup_entrepreneurship_id);
        ...
        const filesArray = Array.isArray(dataItem.startup_supporting_documents) 
          ? dataItem.startup_supporting_documents 
          : [];

        const filesByField = {
          "startup_description": filesArray.length > 0
            ? filesArray.map((f, i) => ({
                id: f.id || `file-startup-${i}`,
        ...
        setInitialData({
          content: { 
            startup_description: dataItem.startup_description || "" 
          },
          tableData: [],
        ...
        console.log("📭 Criterion9_5Form - No existing data found, showing blank form");
        setStartupEntrepreneurshipId(null);
  ...
  }, [nba_accredited_program_id, nba_criteria_sub_level2_id, nba_contributor_allocation_id, otherStaffId, propStartupEntrepreneurshipId, editMode]);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        startup_description: formData.content?.startup_description || "",
        startup_supporting_documents: filesWithCategory
      ...
      console.log("🟠 Criterion9_5Form - Save payload:", payload);

      let response;
      const hasExistingEntry = startupEntrepreneurshipId || propStartupEntrepreneurshipId;

      if (hasExistingEntry) {
        // Update existing record
        const idToUse = startupEntrepreneurshipId || propStartupEntrepreneurshipId;
        console.log("🔄 Criterion9_5Form - Updating existing entry with ID:", idToUse);
        response = await newnbaCriteria9Service.updateCriteria9('9.5', idToUse, payload);
      } else {
        // Create new record
        console.log("🆕 Criterion9_5Form - Creating new entry");
        response = await newnbaCriteria9Service.saveCriteria9Data('9.5', payload);
      }
      ...
      Object.keys(formData.filesByField || {}).forEach(field => {
        ...
        updatedFilesByField[field] = files.map(file => {
          ...
          const savedFile = payload.startup_supporting_documents?.find(
      ...
      if (response?.startup_entrepreneurship_id && !startupEntrepreneurshipId) {
        setStartupEntrepreneurshipId(response.startup_entrepreneurship_id);
      }
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Criterion 9.5 saved successfully
        </SweetAlert>
      );
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria9Service.deleteCriteria9('9.5', startupEntrepreneurshipId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Startup and Entrepreneurship record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setStartupEntrepreneurshipId(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <GenericCriteriaForm
        ...
        onSave={handleSave}
        onDelete={startupEntrepreneurshipId ? handleDelete : null}
        isCompleted={completed}
        isContributorEditable={isContributorEditable}
        saving={saving}
        hasExistingData={!!startupEntrepreneurshipId}
...
export default Criterion9_5Form;