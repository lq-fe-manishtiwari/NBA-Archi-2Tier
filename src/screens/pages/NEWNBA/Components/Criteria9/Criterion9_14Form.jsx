...
const Criterion9_14Form = ({
  ...
  otherStaffId = null,
  editMode = false,
  outreach_activities_id: propOutreachActivitiesId = null,
}) => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [outreachActivitiesId, setOutreachActivitiesId] = useState(null);
    ...
    fields: [
      {
        ...
        label: "Provide details of outreach activities such as community service, Unnat Bharat Abhiyan, social internship and society connect activities undertaken by the students and their achievements.",
        marks: 5,
        type: "textarea"
      }
    ]
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🎯 Criterion9_14Form - Loading data with:");
      ...
      console.log("  - propOutreachActivitiesId:", propOutreachActivitiesId);
      ...
      console.log("🎯 Criterion9_14Form - Final staffId:", staffIdToUse);

      if (!staffIdToUse) {
        console.log("❌ Criterion9_14Form - No staffId found, using empty data");
        setInitialData({ 
          content: { outreach_description: "" }, 
          tableData: [], 
          filesByField: {} 
        });
        setLoading(false);
      ...
      console.log("📡 Criterion9_14Form - Making API call with:");
      ...
      console.log("📊 Criterion9_14Form - Raw API Response:", response);
      ...
      if (Array.isArray(response)) {
        if (response.length > 0) {
          dataItem = response[0];
          console.log("📦 Criterion9_14Form - Extracted first item from array:", dataItem);
        } else {
          console.log("📭 Criterion9_14Form - Empty array response, no data found");
        }
      } else {
        dataItem = response;
      }
      ...
      if (dataItem && dataItem.outreach_activities_id) {
        console.log("✅ Criterion9_14Form - Found existing data");
        ...
        // Set ID
        setOutreachActivitiesId(dataItem.outreach_activities_id);
        ...
        const filesArray = Array.isArray(dataItem.outreach_documents) 
          ? dataItem.outreach_documents 
          : [];

        const filesByField = {
          "outreach_description": filesArray.length > 0
            ? filesArray.map((f, i) => ({
                id: f.id || `file-outreach-${i}`,
        ...
        setInitialData({
          content: { 
            outreach_description: dataItem.outreach_description || "" 
          },
          tableData: [],
        ...
        console.log("📭 Criterion9_14Form - No existing data found, showing blank form");
        setOutreachActivitiesId(null);
  ...
  }, [nba_accredited_program_id, nba_criteria_sub_level2_id, nba_contributor_allocation_id, otherStaffId, propOutreachActivitiesId, editMode]);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        outreach_description: formData.content?.outreach_description || "",
        outreach_documents: filesWithCategory
      ...
      console.log("🟠 Criterion9_14Form - Save payload:", payload);

      let response;
      const hasExistingEntry = outreachActivitiesId || propOutreachActivitiesId;

      if (hasExistingEntry) {
        // Update existing record
        const idToUse = outreachActivitiesId || propOutreachActivitiesId;
        console.log("🔄 Criterion9_14Form - Updating existing entry with ID:", idToUse);
        response = await newnbaCriteria9Service.updateCriteria9('9.14', idToUse, payload);
      } else {
        // Create new record
        console.log("🆕 Criterion9_14Form - Creating new entry");
        response = await newnbaCriteria9Service.saveCriteria9Data('9.14', payload);
      }
      ...
      Object.keys(formData.filesByField || {}).forEach(field => {
        ...
        updatedFilesByField[field] = files.map(file => {
          ...
          const savedFile = payload.outreach_documents?.find(
      ...
      if (response?.outreach_activities_id && !outreachActivitiesId) {
        setOutreachActivitiesId(response.outreach_activities_id);
      }
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Criterion 9.14 saved successfully
        </SweetAlert>
      );
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria9Service.deleteCriteria9('9.14', outreachActivitiesId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Outreach Activities record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setOutreachActivitiesId(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <GenericCriteriaForm
        ...
        onSave={handleSave}
        onDelete={outreachActivitiesId ? handleDelete : null}
        isCompleted={completed}
        isContributorEditable={isContributorEditable}
        saving={saving}
        hasExistingData={!!outreachActivitiesId}
...
export default Criterion9_14Form;