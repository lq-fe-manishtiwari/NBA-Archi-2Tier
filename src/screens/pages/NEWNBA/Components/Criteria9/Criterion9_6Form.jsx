...
const Criterion9_6Form = ({
  ...
  otherStaffId = null,
  editMode = false,
  governance_transparency_id: propGovernanceTransparencyId = null,
}) => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [governanceTransparencyId, setGovernanceTransparencyId] = useState(null);
    ...
    fields: [
      {
        ...
        label: "9.6.1. Governing Body, Administrative Setup, Functions of Various Bodies, Service Rules, Recruitment procedures and Promotion Policies (10)",
        marks: 10,
        type: "textarea"
      },
      {
        name: "transparency_details",
        label: "9.6.2. Transparency (05)",
        marks: 5,
        type: "textarea"
      }
    ]
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🎯 Criterion9_6Form - Loading data with:");
      ...
      console.log("  - propGovernanceTransparencyId:", propGovernanceTransparencyId);
      ...
      console.log("🎯 Criterion9_6Form - Final staffId:", staffIdToUse);

      if (!staffIdToUse) {
        console.log("❌ Criterion9_6Form - No staffId found, using empty data");
        setInitialData({ 
          content: { 
            governance_details: "",
      ...
      console.log("📡 Criterion9_6Form - Making API call with:");
      ...
      console.log("📊 Criterion9_6Form - Raw API Response:", response);
      ...
      if (Array.isArray(response)) {
        if (response.length > 0) {
          dataItem = response[0];
          console.log("📦 Criterion9_6Form - Extracted first item from array:", dataItem);
        } else {
          console.log("📭 Criterion9_6Form - Empty array response, no data found");
        }
      } else {
        dataItem = response;
      }
      ...
      if (dataItem && dataItem.governance_transparency_id) {
        console.log("✅ Criterion9_6Form - Found existing data");
        ...
        // Set ID
        setGovernanceTransparencyId(dataItem.governance_transparency_id);
        ...
        const filesArray = Array.isArray(dataItem.governing_file_uploads) 
          ? dataItem.governing_file_uploads 
          : [];

        const filesByField = {
          "governance_details": filesArray.filter(f => f.category === 'governing_body').length > 0
          ...
          "transparency_details": filesArray.filter(f => f.category === 'transparency').length > 0
        ...
        setInitialData({
          content: { 
            governance_details: dataItem.governance_details || "",
            transparency_details: dataItem.transparency_details || ""
          },
          tableData: [],
        ...
        console.log("📭 Criterion9_6Form - No existing data found, showing blank form");
        setGovernanceTransparencyId(null);
  ...
  }, [nba_accredited_program_id, nba_criteria_sub_level2_id, nba_contributor_allocation_id, otherStaffId, propGovernanceTransparencyId, editMode]);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const filesWithCategory = Object.keys(formData.filesByField || {}).flatMap(
        (field) =>
          (formData.filesByField[field] || []).map((file) => ({
            ...file,
            category: field === 'governance_details' ? 'governing_body' : 'transparency',
      ...
      const payload = {
        ...
        governance_details: formData.content?.governance_details || "",
        transparency_details: formData.content?.transparency_details || "",
        governing_file_uploads: filesWithCategory
      ...
      console.log("🟠 Criterion9_6Form - Save payload:", payload);

      let response;
      const hasExistingEntry = governanceTransparencyId || propGovernanceTransparencyId;

      if (hasExistingEntry) {
        // Update existing record
        const idToUse = governanceTransparencyId || propGovernanceTransparencyId;
        console.log("🔄 Criterion9_6Form - Updating existing entry with ID:", idToUse);
        response = await newnbaCriteria9Service.updateCriteria9('9.6', idToUse, payload);
      } else {
        // Create new record
        console.log("🆕 Criterion9_6Form - Creating new entry");
        response = await newnbaCriteria9Service.saveCriteria9Data('9.6', payload);
      }
      ...
      Object.keys(formData.filesByField || {}).forEach(field => {
        ...
        updatedFilesByField[field] = files.map(file => {
          ...
          const savedFile = payload.governing_file_uploads?.find(
      ...
      if (response?.governance_transparency_id && !governanceTransparencyId) {
        setGovernanceTransparencyId(response.governance_transparency_id);
      }
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Criterion 9.6 saved successfully
        </SweetAlert>
      );
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria9Service.deleteCriteria9('9.6', governanceTransparencyId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Governance and Transparency record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setGovernanceTransparencyId(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <GenericCriteriaForm
        ...
        onSave={handleSave}
        onDelete={governanceTransparencyId ? handleDelete : null}
        isCompleted={completed}
        isContributorEditable={isContributorEditable}
        saving={saving}
        hasExistingData={!!governanceTransparencyId}
...
export default Criterion9_6Form;