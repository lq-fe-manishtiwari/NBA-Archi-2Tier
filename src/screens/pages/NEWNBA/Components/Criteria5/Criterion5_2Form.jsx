...
const Criterion5_2Form = ({
  ...
  console.log("🔵 Criterion5_2Form props:", {
    cycle_sub_category_id,
    other_staff_id,
    isEditable
  });
...
• Faculty cadre and qualification and experience should be as per AICTE/UGC norms.`
  ...
  const loadData = useCallback(async () => {
    ...
    console.log("🎯 Criterion5_2Form - Final staffId:", staffIdToUse);
    console.log("🎯 Is Coordinator View:", isCoordinatorView);
    console.log("🎯 Current User Staff ID:", currentUserStaffId);

    if (!staffIdToUse) {
      console.log("❌ Criterion5_2Form - No staffId found, using empty data");
      setInitialData({
    ...
    try {
      setLoading(true);
      console.log("📡 Criterion5_2Form - Making API call with:");
      ...
      if (data.cadre_Ratio_id || data.id) {
        ...
        const formattedTableData = (data.faculty_cadre_ratio_data || []).map((item, index) => ({
          id: `row-${index}`,
          year: item.year || "",
        ...
        // Handle files
        let filesByField = {};
        if (data.faculty_cadre_ratio_document && Array.isArray(data.faculty_cadre_ratio_document)) {
          data.faculty_cadre_ratio_document.forEach(doc => {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const faculty_cadre_ratio_document = filesWithCategory
        .filter((f) => {
      ...
      console.log("faculty_cadre_ratio_document:", faculty_cadre_ratio_document);
      ...
      const payload = {
        ...
        faculty_cadre_ratio_data: formattedTableData,
        faculty_cadre_ratio_document: faculty_cadre_ratio_document,
      };
      ...
      if (recordId) {
        ...
        response = await newnbaCriteria5Service.updateCriteria5_2_Data(recordId, payload,staffIdToSave);
        console.log("✅ Update response:", response);
        setSuccessMessage("Section updated successfully!");
      } else {
        // Create new record
        response = await newnbaCriteria5Service.saveCriteria5_2_Data(payload,staffIdToSave);
...
export default Criterion5_2Form;