...
const loadData = async () => {
  ...
  try {
    const res = await newnbaCriteria5Service.getCriteria5_9_Data(
      cycle_sub_category_id,
      currentOtherStaffId
    );
  ...
  setInitialData({
    content: {
      // ✅ Editor content
      "5.9": d.visiting_faculty_description || "",
    },
    tableData: {},
    po_pso_id: d.id || null,
    filesByField: {
      // ✅ Visiting faculty files
      "5.9": (d.visiting_faculty_document || []).length > 0
  ...
  const loadContributorsData = async () => {
    ...
    setCardLoading(true);
    try {
      const contributorsResponse = await newnbaCriteria5Service.getAllCriteria5_9_Data?.(cycle_sub_category_id);
      if (onStatusChange) {
 ...
 const handleSave = async (formData) => {
  ...
  try {
    // Flatten uploaded files for 5.9
    const visitingFacultyFiles =
      (formData.filesByField?.["5.9"] || [])
    ...
    const payload = {
      ...
      // ✅ editor content
      visiting_faculty_description: formData.content?.["5.9"] || "",

      // ✅ uploaded documents
      visiting_faculty_document: visitingFacultyFiles,
    };

    console.log("FINAL API PAYLOAD (5.9):", payload);

    if (initialData?.po_pso_id) {
      await newnbaCriteria5Service.putCriteria5_9_Data(
        initialData.po_pso_id,
        payload,
        currentOtherStaffId
      );
    } else {
      await newnbaCriteria5Service.saveCriteria5_9_Data(
        payload,
        currentOtherStaffId
      );
    }
