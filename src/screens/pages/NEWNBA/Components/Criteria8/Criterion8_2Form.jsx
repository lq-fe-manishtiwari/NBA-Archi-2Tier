...
const Criterion8_2Form = ({
  ...
  const createEmptyFile = () => ({
    id: `file-8.2.1-default-${Date.now()}`,
    description: "",
    file: null,
    filename: "",
    s3Url: "",
  ...
  const processData = (d) => {
    ...
    setInitialData({
      content: {
        "8.2.1": d.student_support_description || ""
      },
      tableData: {},
      filesByField: {
        "8.2.1": ensureFileSlot(files)
      }
    });
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const documents = (formData.filesByField?.["8.2.1"] || [])
        .filter(f => f.s3Url)
        .map(f => ({
          filename: f.filename,
      ...
      const payload = {
        ...
        student_support_description: formData.content?.["8.2.1"] || "",
  ...
  const handleDelete = () => {
    ...
    setAlert(
      ...
      >
        This will permanently delete Criterion 8.2 data
      </SweetAlert>
    );
  };
