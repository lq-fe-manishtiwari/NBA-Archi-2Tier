// src/screens/pages/NEWNBA/Components/Criteria8/Criterion8_5Form.jsx
...
const Criterion8_5Form = ({
  ...
  const config = {
    ...
    fields: [
      {
        name: "career_guidance_details",
        type: "textarea",
        rows: 6,
        placeholder: "Describe career guidance, training programs, and placement activities...",
      }
    ]
  };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!staffIdToUse) {
        setInitialData({
          content: { career_guidance_details: "" },
          tableData: [],
          filesByField: {}
        });
        setLoading(false);
      ...
      const response = await newnbaCriteria8Service.getCriteria8_5_Data(
        nba_criteria_sub_level2_id,
        staffIdToUse
      );
      ...
      if (dataItem && dataItem.career_guidance_id) {
        ...
        setProfessionalDevelopmentId(dataItem.career_guidance_id);
        ...
        const filesByField = {
          "career_guidance_details": filesArray.length > 0
            ? filesArray.map((f, i) => ({
                id: f.id || `file-career-${i}`,
        ...
        setInitialData({
          content: { career_guidance_details: dataItem.career_guidance_details || "" },
          tableData: response,
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        career_guidance_details: formData.content?.career_guidance_details || "",
      ...
      if (hasExistingEntry) {
        ...
        response = await newnbaCriteria8Service.updateData8_5(idToUse, payload);
      } else {
        response = await newnbaCriteria8Service.saveCriteria8_5_Data(payload);
      }
      ...
      if (response?.career_guidance_id && !professionalDevelopmentId) {
        setProfessionalDevelopmentId(response.career_guidance_id);
        setHasExistingData(true);
      }
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Criterion 8.5 saved successfully
        </SweetAlert>
      );
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria8Service.deleteData8_5(professionalDevelopmentId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Career Guidance record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setProfessionalDevelopmentId(null);
...
export default Criterion8_5Form;