// src/screens/pages/NEWNBA/Components/Criteria8/Criterion8_3Form.jsx
...
const Criterion8_3Form = ({
    ...
    fields: [
      {
        ...
        type: "textarea",
        rows: 6,
        placeholder: "Describe feedback on facilities...",
      }
    ]
  };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (dataItem && dataItem.feedback_facilities_id) {
        ...
        setProfessionalDevelopmentId(dataItem.feedback_facilities_id);
        ...
        const filesByField = {
          "feedback_details": filesArray.length > 0
            ? filesArray.map((f, i) => ({
                id: f.id || `file-feedback-${i}`,
        ...
        setInitialData({
          content: { feedback_details: dataItem.feedback_details || "" },
          tableData: response,
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        feedback_details: formData.content?.feedback_details || "",
      ...
      if (response?.feedback_facilities_id && !professionalDevelopmentId) {
        setProfessionalDevelopmentId(response.feedback_facilities_id);
        setHasExistingData(true);
      }
    ...
    setAlert(
      <SweetAlert
        ...
        onConfirm={async () => {
          setAlert(null);
          try {
            await newnbaCriteria8Service.deleteData8_3(professionalDevelopmentId);
            setAlert(
              <SweetAlert
                success
                title="Deleted!"
                ...
                onConfirm={() => setAlert(null)}
              >
                Feedback on Facilities record deleted successfully
              </SweetAlert>
            );
            await loadData();
            setProfessionalDevelopmentId(null);
