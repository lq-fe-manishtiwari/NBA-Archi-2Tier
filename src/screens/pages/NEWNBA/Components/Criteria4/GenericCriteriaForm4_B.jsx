...
const GenericCriteriaForm4_B = ({
  ...
  const handleFileChange = async (fieldName, index, newFile) => {
    ...
    try {
      ...
      setFilesByField(prev => ({
        ...prev,
        [fieldName]: prev[fieldName].map((f, i) => i === index ? { ...f, s3Url, filename: newFile.name, uploading: false } : f)
      }));
      console.log("✅ 4B Updated filesByField after upload");
...
export default GenericCriteriaForm4_B;
