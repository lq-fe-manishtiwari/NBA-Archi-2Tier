...
// Generic Table
const GenericTable = ({ columns, data = [], onChange, disabled }) => {
                ...
                {safeData.map((row, i) => (
                  <Draggable key={row.id} draggableId={row.id.toString()} index={i} isDragDisabled={disabled}>
                    {(provided) => (
                      ...
                      >
                        <td className="p-3">
                          <div {...provided.dragHandleProps} className="cursor-grab">☰</div>
                        </td>
...
const GenericCriteriaForm4_C = ({
  ...
  const [filesByField, setFilesByField] = useState(() => {
    ...
    fields.forEach((field) => {
      init[field.name] = [{ id: `file-${Date.now()}-${field.name}-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }];
    });
    return init;
  });

  React.useEffect(() => {
    console.log("🔄 GenericCriteriaForm4_C useEffect triggered with initialData:", initialData);
    if (initialData) {
      console.log("✅ Setting formValues:", initialData.content);
      console.log("✅ Setting tableData:", initialData.tableData);
      console.log("✅ Setting filesByField:", initialData.filesByField);
  ...
  const handleFileChange = async (fieldName, index, newFile) => {
    ...
    try {
      ...
      const s3Url = typeof resData === 'string' ? resData : (resData?.downloadPath || resData?.url || resData?.data?.downloadPath || resData?.data?.url || "");
          ...
          <div key={field.name} className="space-y-6">
            ...
            <div className="mt-6 p-6 bg-gray-50 rounded-xl border">
              <div className="flex justify-between items-center mb-4">
                ...
                {isEditMode && !isCompleted && filesByField[field.name]?.length > 1 && (
                    <button
...
export default GenericCriteriaForm4_C;
