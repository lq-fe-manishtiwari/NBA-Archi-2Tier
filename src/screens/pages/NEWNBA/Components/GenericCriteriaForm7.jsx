// src/screens/pages/NEWNBA/GenericCriteriaForm7.jsx
...
const GenericTable = ({
  ...
  const isSevenFive = fieldId === "7.5";

  // ✅ Fixed NBA rows for Section 7.5
  const fixedSevenFiveRows = [
    {
      item: "National Level Entrance Examination (JEE Main / Other)",
      cay: "",
      caym1: "",
      caym2: "",
    },
    ...
    {
      item:
        "Average percentage of all students of CBSE / Any other Board Result of admitted students",
      cay: "",
      caym1: "",
      caym2: "",
    },
  ...
  // ✅ Always enforce NBA structure for 7.5
  const safeData = isSevenFive
    ? fixedSevenFiveRows.map((row, index) => ({
        ...row,
        ...(data[index] || {}),
        id: data[index]?.id || `row-${index}`,
      }))
    : data.length > 0
    ? data.map((r, i) => ({ ...r, id: r.id || `row-${i}` }))
    : [];

  const isTitleRow = (index) =>
    isSevenFive ? index === 0 || index === 4 : readOnlyRows.includes(index);

  const handleChange = (rowIndex, field, value) => {
    if (disabled || isTitleRow(rowIndex)) return;
  ...
  const addRow = () => {
    if (isSevenFive) return; // 🚫 NBA rule
    onChange([
      ...safeData,
      {
        id: `row-${Date.now()}`,
  ...
  const removeRow = (index) => {
    if (disabled || isSevenFive || isTitleRow(index)) return;
    onChange(safeData.filter((_, i) => i !== index));
  };

  const onDragEnd = (result) => {
    if (!result.destination || disabled || isSevenFive) return;
    if (isTitleRow(result.source.index)) return;
        ...
        <table className="w-full border-collapse bg-white shadow rounded-lg">
          <thead>
            <tr className="bg-blue-700 text-white">
              ...
              {!disabled && !isSevenFive && <th className="w-16"></th>}
            </tr>
          </thead>

          <Droppable droppableId={`droppable-${fieldId}`}>
            {(provided) => (
              <tbody ref={provided.innerRef} {...provided.droppableProps}>
                {safeData.map((row, index) => {
                  const readOnly = disabled || isTitleRow(index);
                  ...
                  return (
                    <Draggable
                      ...
                      draggableId={row.id}
                      index={index}
                      isDragDisabled={disabled || isSevenFive || isTitleRow(index)}
                    >
                      {(dragProvided) => (
                        <tr
                        ...
                        >
                          <td className="p-3">
                            {!disabled && !isSevenFive && !isTitleRow(index) && (
                              <div
                          ...
                          {columns.map((col) => (
                            <td key={col.field} className="p-3">
                              <input
                                ...
                                readOnly={readOnly}
                                onChange={(e) =>
                                  handleChange(index, col.field, e.target.value)
                                }
...
const GenericCriteriaForm7 = ({
  ...
  const [isEditMode, setIsEditMode] = useState(!isCompleted && !hasExistingData);
  ...
  useEffect(() => {
    console.log("🔄 GenericCriteriaForm useEffect triggered with initialData:", initialData);
    if (initialData?.content) {
      console.log("📝 Updating formValues with:", initialData.content);
      setFormValues(initialData.content);
    }
    if (initialData?.tableData) {
      console.log("📊 Updating tableData with:", initialData.tableData);
      setTableData(initialData.tableData);
    }
    if (initialData?.filesByField) {
      console.log("📁 Updating filesByField with:", initialData.filesByField);
  ...
  const handleFileChange = async (fieldName, index, newFile) => {
    ...
    try {
      ...
      // Extract S3 URL from response - use downloadPath if available, otherwise url
              ...
              <>
                ...
                {field.hasTable ? (
                  <GenericTable
                    ...
                    data={Array.isArray(tableData) ? tableData : (tableData[field.name] || [])}
                    onChange={(newData) => {
                      // Apply callback transformation if provided
                      const processedData = onTableDataChange ? onTableDataChange(field.name, newData) : newData;

                      if (Array.isArray(tableData)) {
                        setTableData(processedData);
                      } else {
                        setTableData((prev) => ({ ...prev, [field.name]: processedData }));
                      }
                    }}
                    disabled={!isEditMode}
                    fieldId={field.name}
                    readOnlyRows={field.tableConfig?.readOnlyRows || []}
                    tableTitle={field.tableConfig?.title}
                  />
                ) : (
                ...
                {/* Supporting Documents - Always show section */}
                {!isCompleted && (
  ...
  />
                      ...
                      /* View Mode - Show all files including empty ones */
...
export default GenericCriteriaForm7;
