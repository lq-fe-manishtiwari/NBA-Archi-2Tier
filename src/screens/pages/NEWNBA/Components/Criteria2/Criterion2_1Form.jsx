...
const Criterion2_1Form = ({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🟠 Criterion2_1Form - Loading data:");
      ...
      console.log("🟢 Criterion2_1Form - Raw API Response:", rawResponse);
      console.log("🟢 Criterion2_1Form - Processed Data:", d);

      // If API returns nothing, show blank form
      setTeachingLearningQualityId(d.teaching_learning_quality_id || null);

      setInitialData({
        content: { "2.1": d.quality_processes_description || "" },
        tableData: [],
        filesByField: {
          "2.1": (d.quality_process_documents || []).length > 0
            ...
            : [{ id: `file-2.1-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }]
        }
      });

    } catch (err) {
      console.warn("❌ Criterion2_1Form - API failed or returned 404, showing blank form", err);
      setTeachingLearningQualityId(null);
      setInitialData({
        content: { "2.1": "" },
        tableData: [],
        filesByField: {
          "2.1": [{ id: `file-2.1-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }]
        }
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        cycle_sub_category_id,
        quality_processes_description: formData.content["2.1"] || "",
      ...
      console.log("🟠 Criterion2_1Form - Save payload:", payload);
