...
const Criterion2_4Form = ({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("🟠 Criterion2_4Form - Loading data:");
      ...
      console.log("🟢 Criterion2_4Form - Raw API Response:", rawResponse);
      console.log("🟢 Criterion2_4Form - Processed Data:", d);

      // If API returns nothing, show blank form
      setTeachingLearningQualityId(d.teaching_learning_quality_id || null);

      setInitialData({
        content: { "2.4": d.quality_processes_description || "" },
        tableData: [],
        filesByField: {
          "2.4": (d.quality_process_documents || []).length > 0
            ...
            : [{ id: `file-2.4-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }]
        }
      });

    } catch (err) {
      console.warn("❌ Criterion2_4Form - API failed or returned 404, showing blank form", err);
      setTeachingLearningQualityId(null);
      setInitialData({
        content: { "2.4": "" },
        tableData: [],
        filesByField: {
          "2.4": [{ id: `file-2.4-0`, description: "", file: null, filename: "", s3Url: "", uploading: false }]
        }
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        cycle_sub_category_id,
        quality_processes_description: formData.content["2.4"] || "",
      ...
      console.log("🟠 Criterion2_4Form - Save payload:", payload);
      ...
      setAlert(
        ...
        >
          Criterion 2.4 saved successfully
        </SweetAlert>
      );
      ...
      <SweetAlert
        ...
        onConfirm={async () => {
          ...
          // Handle plain text response
            let message = "Teacher Learning record deleted successfully.";
...
export default Criterion2_4Form;
