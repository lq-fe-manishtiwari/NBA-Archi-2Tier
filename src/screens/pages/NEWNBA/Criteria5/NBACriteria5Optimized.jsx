...
import CriterionForm5 from "../Components/Criteria5/CriterionForm5";
...
const NBACriteria5Optimized = () => {
  ...
  const s3DocumentUrl =
    "https://wccdm.s3.ap-south-1.amazonaws.com/nba/1765294023-merged_5.png";
  ...
  const getCardConfig = (sectionName) => {
    ...
    if (cardSupportedSections.includes(effectiveSectionCode)) {
      return {
        title: `${effectiveSectionCode} Entry`,
        statusField: "approval_status",
        userField: "other_staff_id",
        nameFields: ["firstname", "lastname"],
        idField: getSectionIdField(sectionName),
  ...
  const getSectionIdField = (sectionName) => {
    const sectionCode = sectionName ? sectionName.match(/5\.(\d+)/)?.[0] : null;
    const isFiveA = sectionName && (sectionName.startsWith("5A:") || sectionName === '5A');
  ...
  const getApiService = (sectionName) => {
    console.log("Determining API service for sectionName:", sectionName);
    const sectionCode = sectionName ? sectionName.match(/5\.(\d+)/)?.[0] : null;
    const isFiveA = sectionName && (sectionName.startsWith("5A:") || sectionName === '5A');
    const effectiveSection = isFiveA ? '5A' : sectionCode;

    if (effectiveSection) {
      return {
        updateCardStatus: (statusPayload, approverId) => {
          switch (effectiveSection) {
            case '5.1':
              return newnbaCriteria5Service.updateCriteria5_1_ApprovalStatus(approverId, statusPayload);
            case '5.2':
              return newnbaCriteria5Service.updateCriteria5_2_ApprovalStatus(approverId, statusPayload);
            case '5.3':
              return newnbaCriteria5Service.updateCriteria5_3_ApprovalStatus(approverId, statusPayload);
            case '5.4':
        ...
        getCardData: (cycleSubCategoryId) => {
          switch (effectiveSection) {
            ...
            case '5.3':
              return newnbaCriteria5Service.getallCardDetails5_3Data(cycleSubCategoryId);
            case '5.4':
  ...
  useEffect(() => {
    ...
    console.log("🔍 CRITERIA 5 NAVIGATION DEBUG:");
    ...
    if (!nbaAccreditedProgramId || !nbaCriteriaId || !nbaCriteriaSubLevel1Id) {
      console.log("❌ CRITERIA 5: Missing navigation parameters, redirecting to view-part-b");
  ...
  const fetchCardDetails = async (cycleSubCategoryId, sectionName = "5.1") => {
    console.log("🔍 CRITERIA 5 DEBUG: fetchCardDetails called with cycleSubCategoryId:", cycleSubCategoryId, "sectionName:", sectionName);

    try {
      // Determine which service to use based on section
      const sectionCode = sectionName ? sectionName.match(/5\.(\d+)/)?.[0] : '5A';
      const isFiveA = sectionName && (sectionName.startsWith("5A:") || sectionName === '5A');
      const effectiveSection = isFiveA ? '5A' : sectionCode;

      const cardService = getApiService(effectiveSection)?.getCardData; // Pass full name
      const idField = getSectionIdField(effectiveSection); // Pass full name

      if (!cardService) {
        console.log("❌ No card service found for section:", effectiveSection);
        return [];
      }

      // Fetch contributor cards
      console.log("📡 CRITERIA 5 DEBUG: Fetching contributor cards...");
      console.log("Using sectionCode:", effectiveSection, "and cycleSubCategoryId:", cycleSubCategoryId);
      ...
      if (currentStaffId) {
        try {
          ...
          let coordinatorData;
          console.log("sectionCode------", effectiveSection)
          // Use appropriate service based on section
          switch (effectiveSection) {
            case '5.1':
              coordinatorData = await newnbaCriteria5Service.getCriteria5_1_Data(cycleSubCategoryId, currentStaffId);
              break; // This was missing
            case '5.2':
              coordinatorData = await newnbaCriteria5Service.getCriteria5_2_Data(cycleSubCategoryId, currentStaffId);
              break;
            case '5.3':
              coordinatorData = await newnbaCriteria5Service.getCriteria5_3_Data(cycleSubCategoryId, currentStaffId);
              break;
            case '5.4':
              coordinatorData = await newnbaCriteria5Service.getCriteria5_4_Data(cycleSubCategoryId, currentStaffId);
              break;
            case '5.5':
  ...
  const toggleSection = async (id) => {
    ...
    if (!wasExpanded) {
      ...
      console.log("🔍 DEBUG: Found indicator for section:", indicator);

      if (indicator && indicator.rawName) {

        // Original regex match
        const sectionCode = indicator.rawName.match(/5\.(\d+)/)?.[0];
        console.log("📋 DEBUG: Section code extracted:", sectionCode);

        // NEW SPECIAL CASE FOR 5A:
  ...
  const cleanDisplayName = (rawName) => {
    ...
    return rawName.trim().replace(/^5\.\d+\s*-?\s*/i, "").trim() || "Untitled Section";
  };
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const response = await ReportService.getReportCri5(cycleSubCategoryIds);
      ...
      link.href = url;
      link.download = `Criterion5_Report_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  if (loading) {
    return <CriteriaLoader criterionNumber={5} progress={progress} />;
  }
          ...
          <div className="space-y-6">
            {keyIndicators.length === 0 ? (
              ...
              keyIndicators.map((indicator, index) => {
                // Extract the actual section number from the rawName instead of using index
                const rawName = indicator.rawName || "";
                const sectionMatch = rawName.match(/^(\d+\.\d+)/);
                const actualSectionCode = sectionMatch ? sectionMatch[1] : `5.${index + 1}`;
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div>
                          ...
                          {!rawName.includes('5A') && actualSectionCode.includes('.') && (
                      ...
                      <div className="px-6 py-8 bg-gray-50 border-t border-gray-200">
                        {isContributor ? (
                                ...
                                <div className="space-y-4">
                                  ...
                                  {selectedCard && selectedCard.cycleSubCategoryId == subLevel2Id && (
                                    <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-6">
                                      {console.log("🎯 CRITERIA 5 - FORM IS RENDERING! This div should be visible.")}
                                      ...
                                      <CriterionForm5
                                        ...
                                        isSubCoordinator={isSubCoordinator} // This was missing
                                        // Pass the dynamic ID field
                                        cardItem={selectedCard.cardData}
                                        {...{ [getSectionIdField(actualSectionCode)]: selectedCard[getSectionIdField(actualSectionCode)] || null }}
                                      />
                                    </div>
                                  )}
                                </div>
                          ...
                          (() => {
                            ...
                            const otherStaffId = userInfo.other_staff_id || userInfo2?.rawData?.other_staff_id || null;
...
export default NBACriteria5Optimized;