...
export default function CategoryPage() {
  ...
  const fetchDivisions = () => {
    ...
    CategoryService.getCategory()
      .then(response => {
        ...
        // Backend returns category_name, so rename for UI compatibility
        const updated = (response || []).map((item) => ({
          ...item,
          name: item.category_name   // 👈 IMPORTANT FIX
        }));

        setDivisions(updated);
        setFilteredDivisions(updated);
        setLoading(false);
      })
      .catch(err => {
  ...
  const handleDeleteClick = (div) => {
    setDeleteAlert({ show: true, id: div.id, name: div.name });
  };

  const handleConfirmDelete = () => {
    CategoryService.deleteCategory(deleteAlert.id)
      .then(() => {
        setDivisions(divisions.filter((d) => d.id !== deleteAlert.id));
  ...
  return (
    <div className="p-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
        ...
        <button
          onClick={() => navigate("/academics/category/add")}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <div className="flex justify-center gap-2">
                              <button
                                ...
                                onClick={() => navigate(`/academics/category/edit/${div.id}`)}
                              >
                      ...
                      <tr>
                        <td colSpan="2" className="px-6 py-8 text-center text-gray-500">
                          {searchTerm
                            ...
                            : "No divisions found. Please add a new one."}
                        </td>
                      </tr>
                    )}
                  </tbody>
