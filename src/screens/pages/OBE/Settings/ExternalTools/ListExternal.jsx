...
const ListExternal = () => {
  ...
  // Reset page when new course selected
useEffect(() => {
  setCurrentPage(1);
}, [selectedCourse]);
  ...
  useEffect(() => {
    if (selectedCourse) {
      getExternalBySubjectId(selectedCourse);
    } else {
      setToolRows([]);
    }
  }, [selectedCourse]);

  const getExternalBySubjectId = async (subject_id) => {
    try {
      const res = await ExternalService.getExternalBySubjectId(subject_id);

      console.log("API External Response:", res);

      if (Array.isArray(res)) {
        setToolRows(
          res.map(item => ({
            toolName: item.name,
            nba_external_config_id: item.nba_external_config_id,
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="pageheading text-xl font-semibold">External Tools Configuration</h2>
        <Link
          to="/obe/settings/Add_EXTERNAL_TOOLS"
                  ...
                  {/* <td className="p-2 border text-center">
                    <Link
                      ...
                      state={{
                        ...
                        name: row.toolName,
                        nba_external_config_id: row.nba_external_config_id,
                      }}
                    >
