...
const AddExternal = () => {
  ...
  console.log("Received data from ListExternal:", locationState);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (isUpdate) {
      payload.id = locationState.nba_external_config_id;
    }

    try {
      await ExternalService.saveExternal(payload);
      setAlert(
        <SweetAlert
          success
          ...
          onConfirm={() => {
            ...
            window.location.href = `/obe/settings/EXTERNAL_TOOLS`;
          }}
        >
      ...
      setAlert(
        <SweetAlert
          ...
          onConfirm={() => setAlert(null)}
        >
          Failed to save external tool configuration!
        </SweetAlert>
      );
    }
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="pageheading text-xl font-semibold">
          {isUpdate ? "Edit" : "Add"} External Config
        </h1>
        <Link
          to="/obe/settings/EXTERNAL_TOOLS"
